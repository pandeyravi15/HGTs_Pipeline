...
/* OUTPUT FILES*/
 	//rindex=atoi(argv[2]);
       fsegmlist=fopen(argv[2],"a");
	fclusterlist=fopen(argv[3],"a");
...
/*######################################################**********************First step:Segementation*****************************######################################################*/
	...
	{h=4;
		for (i0=h-4;i0>=0;i0-=2)
 		{	i4++;
			group[i0]=1;
			fprintf (fsegmlist,"%s\t%d\t%d\t%d\t%d\n",argv[1],i3,fragments[i0+1],fragments[i0],fragments[i0]-fragments[i0+1]);i3++;
...
/*************************************************************Third step:contiguous+NonContiguous clustering**************************************************************************/	
	...
	{	
		...
		percent = (double)size/length;
		//fprintf (fclusterlist,"%s\t%d\t%d\t%d\t%d\t%d\t%d\t%lf\n",argv[1],a[i0],r[i0],r[i0+1],gp1[i0],r[i0+1]-r[i0],size,percent);  // output cluster of non-contiguous clustering//
