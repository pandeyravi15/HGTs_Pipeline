...
segmentation_output:file after segmentation, segmentation_output=file after clustering;that further need to arrange by ascending order.
*/
...
//global variables
int ttl_oligos=0,hash[200000000]={},length=0,x=0,z=0,v=0,h=0,h3=0,i=0,j=0,g1=0;
int u=0,right_end=0,z0=0,seg_points[100000000]={},fragments[30000000]={},group1[10000000]={},group2[10000000]={},r[30000000],a[30000000],gp[10000000]={},gp1[10000000]={},R[3000000]={},R1[3000000]={};
...
int main(int argc, char *argv[])
{
	...
	int x0=0,i0=0,i1=0,i2=0,i3=1,i8=0,i9=0,i4=0,i5=1,i7=1,j4=1,j5=0,l0=0,g2,exp=0,n1=0,n,j1=0,j6=0,j7=0,j0=0,k0,count,size=0,arr1[20000000],arr2[20000000];
	int group[10000000]={};
	char ch, line[5000],opfile[20],opfile1[20],opfile2[20],largestCluster,percentsize; 
	...
	char *genome=malloc(200000000*sizeof(char)),opf_extn[]=".seg.txt",opf_extn1[]=".clustering1.txt",opf_extn2[]=".clustering2.txt";
/* markove model order */
	rindex=atoi(argv[2]);

/* OUTPUT FILES*/
 	
       //fsegmlist=fopen(argv[6],"a");
	fclusterlist=fopen(argv[6],"a");
	//fnonconlist = fopen(argv[4],"a");
//fprintf (fclusterlist,"%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",filename,Cluster,Segment,Start,End,length,Size,Percent);
...
/*######################################################**********************First step:Segementation*****************************######################################################*/
	...
	{h=4;
		for (i0=h-4;i0>=0;i0-=2)
 		{	i4++;
			group[i0]=1;
			//fprintf (fsegmlist,"%s\t%d\t%d\t%d\t%d\n",argv[1],i3,fragments[i0+1],fragments[i0],fragments[i0]-fragments[i0+1]);
			i3++;
...
/*************************************************************Third step:contiguous+NonContiguous clustering**************************************************************************/	
	...
	{	
		...
		percent = (double)size/length;
		fprintf (fclusterlist,"%s\t%d\t%d\t%d\t%d\t%d\t%d\t%lf\n",argv[1],a[i0],gp1[i0],r[i0],r[i0+1],r[i0+1]-r[i0],size,percent);  // output cluster of non-contiguous clustering//
...
/*#####################################**********************Final step:ReNonContiguous clustering**************************#######################################################*/
	...
	{	
		...
		size =size + (r[i0+1]-r[i0]);
		percent = (double)size/length;
		//fprintf (fnonconlist,"%s\t%d\t%d\t%d\t%d\n",argv[1],R[i0],r[i0],r[i0+1],gp1[i0]);  // FINAL output cluster of clustering//
	}
...
float segment (int *hash,int k1,int k2)
{
	...
	double neff=0.0,sarg=0.0,smax=0.0,sx=0.0,beta=0.0;	
...
if (ttl_oligos==4)	
		{  a=2.7784; b=-7.97084; c=0.0; d=0.80;	}		
                        						
else if (ttl_oligos==16)
		{  a=1.55681; b=-2.19548; c=0.0; d=0.946; }
							
else if (ttl_oligos==64)
		{  a=1.13049; b=-2.44732;c=0.002304; d=1.025173;}
...
if(percent > 0.60){printf ("LargestCluster#%d\tSize:%f\n",j1,percent);}
